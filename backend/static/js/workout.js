...
function clientTrainingProgramHTML(d,cid,groups){
 ...
 if(previewWorkoutDay){
   ...
   selected=`<div class="client-program-selected"><div class="client-program-selected-title"><div><strong>${esc(previewWorkoutDay)}</strong>${dayTitle?`<small class="client-program-day-title">${esc(dayTitle)}</small>`:''}</div><span>${xs.length} ${xs.length===1?'вправа':(xs.length<5?'вправи':'вправ')}</span></div>${exercises}<div class="rir-help" style="margin-top:14px"><strong>Суперсет</strong> — дві вправи виконуються одна за одною без звичайного відпочинку між ними. Відпочинок — після завершення обох вправ.</div><div class="rir-help" style="margin-top:10px"><strong>RIR (Reps In Reserve)</strong> — скільки повторів залишилося б у запасі до відмови. Наприклад, RIR 2 = ти міг би зробити ще приблизно 2 повтори.</div></div>`;
 }
